...
export async function registerUser(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const roleValue = required(formData.get("role"));
  const role: Role = isValidRole(roleValue) ? roleValue : "STUDENT";
  ...
  try {
    ...
    const user = await prisma.user.create({
      data: { name, email, course: course || null, role, passwordHash: await bcrypt.hash(password, 10) }
    });
  ...
  try {
    ...
    const location = await prisma.campusLocation.create({
      data: {
        ...
        type,
        description,
        floor: required(formData.get("floor")) || null,
        block: required(formData.get("block")) || null
      }
    });
...
export async function createLostFoundItem(status: ItemStatus, _prevState: ActionState, formData: FormData): Promise<ActionState> {
