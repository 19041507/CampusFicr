...
export function LocationCard({ location }: { location: CampusLocation }) {
  return (
    <Card className="group h-full transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl">
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-black text-blue-900"><span className="rounded-full bg-blue-50 px-3 py-1">Setor {location.block || "-"}</span><span className="rounded-full bg-blue-50 px-3 py-1">Local interno</span><span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1"><QrCode size={13}/>QR</span></div>
